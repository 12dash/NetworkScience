...
def get_degree_distribution(network, title):
    ...
    p = figure(title=title, plot_width=400, plot_height=400, y_axis_type="log", x_axis_type="log")
...
def generate_tab_year(graphs):
    def get_first_row(temp):
        ...
        year_collab = generate_year_graph(temp.graph_year, f"Collaborations in {i}")
        degree_distribution=get_degree_distribution(temp.graph_year,"Degree Distribution")
    ...
    for i in range(2000, 2021):
        ...
        grid = layout([row_1, generate_year_graph(temp.graph_previous_years, f"Collaborations from 2000 to {i}")])
...
def show_html(Year_Graph, Faculty=None): 
    tabs = generate_tab_year(Year_Graph)
    l = [startingHtml(),tabs]    
    disp = layout(l)
    show(disp)
